...
public class ProgramParser {
   ...
   // private Map<String,>
    ...
    public ProgramParser(String pathname) {
        ...
        generatedInstructionList.add(new Instruction()); //添加一个占位元素
        try {
                ...
                case ident:{   //因子为变量或者函数调用
                    ...
                    if (word.getType() == Symbol.lbracket) { //函数调用
                        ...
                        if (word.getType() != Symbol.rbracket) {
                            ...
                            FunctionCall functionCall = new FunctionCall(val, position,true);
    ...
    */
                ...
                case ident: {
                    ...
                    if (word.getType() == Symbol.lbracket) { //void函数调用
                        ...
                        if (word.getType() != Symbol.rbracket) {
                            ...
                            FunctionCall functionCall = new FunctionCall(val, lastInstructionPosition,false);
                            functionCallList.add(functionCall);
                        }
                        ...
                        if (variable != null && (variable.getScope().equals(functionName)||variable.getScope().equals("global"))) {
                            ...
                            //赋值
                            if (variable.getScope().equals(functionName)) {
                                lastInstructionPosition = generateInstruction(InstructionType.STO, 0,variable.getAddress()); //当前层
                            } else {
                                lastInstructionPosition = generateInstruction(InstructionType.STO, 1,variable.getAddress()); //全局变量，外层
                            }
                        } else {
                ...
                case scanfsym: {
                    ...
                    if (word.getType() != Symbol.lbracket ) {
                        ...
                        if (word.getType() == Symbol.ident) {
                            ...
                            wrongList.add(new Wrong(lexer.getPosition(),1,"缺少标识符(或标识符错误)", 349));
                        }
                        word = lexer.getWord();
                        if (word.getType() != Symbol.rbracket) {
                            wrongList.add(new Wrong(lexer.getPosition(), 3, "缺少')'"));
                ...
                case printfsym: {
                    ...
                    if (word.getType() != Symbol.lbracket ) {
                        ...
                        if (word.getType() == Symbol.ident) {
                            ...
                            wrongList.add(new Wrong(lexer.getPosition(),1,"缺少标识符(或标识符错误)",384));
                        }
                        word = lexer.getWord();
                        if (word.getType() != Symbol.rbracket) {
                            wrongList.add(new Wrong(lexer.getPosition(), 3, "缺少')'"));
    ...
    private void parseSubProgram(String functionName) throws Exception {
        ...
        if (word.getType() != Symbol.lbrace) {
            ...
            if (word.getType() != Symbol.rbrace) {
                wrongList.add(new Wrong(lexer.getPosition(), 6, "缺少'}'",460));
            } else {
                word = lexer.getWord();
            }
            //TODO 函数完毕，填写函数位置
